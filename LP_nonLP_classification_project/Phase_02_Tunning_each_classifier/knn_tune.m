function knn_tune()
% -------------------------------------------------------------------------
% The purpose of the function is to find the BEST value of Neighbor 
% (sensitive parameter) for K nearest neighbor (KNN) classifier (when the 
% K fold cross validation accuracy is maximum). I consider the Neighbor 
% values within the range 1 to 50, and 2-fold cross validation.
% -------------------------------------------------------------------------
% It will generate the accuracy-vs-neighbor graph, and also create an
...
% -------------------------------------------------------------------------
    ...
    file_write = fopen('knn_output.txt','w'); % Open a text file in write mode.
    
    overall_accuracy = [];  % Empty list (to store each accuracy).
    N_seq = [];             % Empty list (to store each Neighbor value).
    ...
    for N = 1: 1: 50 %  Neighbor values from 1 to 50 increamented by 1.
    ...
    plot(N_seq,overall_accuracy(:,1),'--ks','LineWidth',1,'MarkerEdgeColor','k','MarkerFaceColor','g','MarkerSize',4)
    title('Neighbor vs Accuracy');
    xlabel('KNN classifier: Neighbor values');
    ylabel('Overall Classification Accuracy (%)');
    saveas(gca,'knn_tune.bmp');
    ...
    fprintf(file_write,'Value of Neighbor at Max. accuracy : %0.2f\r\n', N_seq(pos,1));
