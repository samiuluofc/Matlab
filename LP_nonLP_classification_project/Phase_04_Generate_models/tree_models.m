function tree_models(bin_ch)
% --------------------------------------------------------------------------
% Generate models (Fold 1 and Fold 2) for Decision tree using 
% selected features. Parameter "bin_ch" is the binary chromosome of the best 
% individual for Decision tree.
% --------------------------------------------------------------------------
    ...
    for k = 1:1:partition % Using K fold cross validation (K = Partition).
        ...
        %D-TREE training. 
        model = ClassificationTree.fit(X_train, Y_train, 'MinLeaf',leaf);
        
        % Saving models in the current directory.
        if(k == 1)
            my_tree1 = model;
            save tree1.mat my_tree1
        end
        if(k == 2)
            my_tree2 = model;
            save tree2.mat my_tree2
        end        
    end
end
